...
describe('FileUploadService', () => {
  ...
  describe('handleFileUpload', () => {
    it('файл должен быть успешно загружен', async () => {
      ...
      expect(mockedFs.mkdir).toHaveBeenCalledWith('public/uploads', { recursive: true });
      expect(mockedFs.writeFile).toHaveBeenCalled();
    });

    it('Если файл не предоставлен, должно быть исключение BadRequestException', async () => {
      await expect(service.handleFileUpload(null as any)).rejects.toThrow(BadRequestException);
      await expect(service.handleFileUpload(null as any)).rejects.toThrow('Нет файла для загрузки');
    });

    it('Должно быть исключение BadRequestException, если тип MIME файла не разрешен', async () => {
      const mockFile = createValidFile();
      mockFile.mimetype = 'application/pdf';
      await expect(service.handleFileUpload(mockFile)).rejects.toThrow(BadRequestException);
    ...
    it('Должно быть исключение BadRequestException, если размер файла превышает 2 MB', async () => {
      ...
      await expect(service.handleFileUpload(mockFile)).rejects.toThrow(BadRequestException);
      await expect(service.handleFileUpload(mockFile)).rejects.toThrow('Размер файла больше допустимого 2 МБ');
    });
    ...
    it('Должен обрабатывать файл без буфера (используя путь) и переименовывать его', async () => {
      ...
      mockedFs.readFile.mockResolvedValue(Buffer.from([0xff, 0xd8, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00]));
      mockedFs.access.mockRejectedValue(new Error('not found'));
      mockedFs.mkdir.mockResolvedValue(undefined);
      ...
      expect(mockedFs.rename).toHaveBeenCalledWith('/tmp/uploaded-file', expect.stringContaining('public/uploads/'));
